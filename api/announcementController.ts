...
**权限要求：**
- 仅限admin角色 POST /announcement/add */
...
**权限要求：**
- 仅限admin角色 POST /announcement/delete */
...
/** 根据ID获取公告 根据ID获取公告详细信息。

**权限要求：**
- 无需登录，所有用户可查询 GET /announcement/get */
...
**权限要求：**
- 无需登录，所有用户可查询 GET /announcement/get/vo */
...
/** 分页获取公告列表 分页查询公告列表（实体类）。

**权限要求：**
- 无需登录，所有用户可查询 POST /announcement/list/page */
export async function listAnnouncementByPage(
  body: API.AnnouncementQueryRequest,
  options?: { [key: string]: any }
) {
  return request<API.BaseResponsePageAnnouncement>("/announcement/list/page", {
    method: "POST",
    headers: {
      "Content-Type": "application/json",
    },
...
**限制条件：**
- 每页最多20条 POST /announcement/list/page/vo */
...
**权限要求：**
- 仅限admin角色 POST /announcement/update */
