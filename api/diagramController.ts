...
/** 获取图表类型列表 获取系统支持的图表类型分类。

**返回内容：**
- 通用领域：流程图、涌道图、甘特图
- 软件工程领域：UML、架构图、网络拓扑图、时序图
- 数据库与信息领域：ER图 GET /diagram/getDiagramTypes */
export async function getDiagramTypes(options?: { [key: string]: any }) {
  return request<API.BaseResponseMapStringListString>(
    "/diagram/getDiagramTypes",
    {
      method: "GET",
      ...(options || {}),
    }
